...
# 获取所有task文件并排序
TASK_FILES=$(find .claude/epics/tms-nlops-demo -name "*.md" | grep -E '00[3-9]|0[1-3][0-9]|04[0-2]' | sort)
...
for file in $TASK_FILES; do
    ...
    if [ -z "$title" ]; then
        echo "  ❌ 无法提取title字段"
        echo "| $filename | - | 无法提取标题 | 失败 |" >> sub-issues-results.md
        FAILED_COUNT=$((FAILED_COUNT + 1))
        continue
    fi

    echo "  ✅ 提取title成功: $title"
    ...
    if command -v gh &> /dev/null; then
        # 使用printf来正确处理body内容中的特殊字符
        issue_result=$(printf '%s' "$body" | gh sub-issue create --title "$title" --body - --parent $PARENT_ISSUE --label enhancement 2>&1)
...
if [ $SUCCESS_COUNT -gt 0 ]; then
    echo ""
    echo "=== 成功创建的Sub-Issues ==="
    grep "成功" sub-issues-results.md | sed 's/| //g' | sed 's/ 成功//'
fi