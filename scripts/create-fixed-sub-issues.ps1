...
foreach ($file in $taskFiles) {
    ...
    Write-Host "  Body长度: $($body.Length) 字符"
    ...
    if (Get-Command gh -ErrorAction SilentlyContinue) {
        try {
            # 创建issue并获取编号
            $issueResult = gh sub-issue create --title $title --body $body --parent $PARENT_ISSUE --label enhancement 2>&1
